...
tcga_res = mapply(function(x,y){
  ...
  pc_data1$Project[which(pc_data1$Project=="XENA_Toil")]="Xena"
...
gtex_res = mapply(function(x, y){
  ...
  pc_data1$Project = gsub("Xena-Toil", "XENA", pc_data1$Project)
...
plots <- lapply( selGenes, function(gene){
  ...
  p1 = myFun(x=temp_gdc, y=temp_xena, plot_title = "Xena vs\n GDC")
...
tcga_data = rbind(
  cbind( Value= tcga_mat[,"corr_XENA_Toil"], Group= rep("Xena", nrow(tcga_mat))) ,
...
group_levels=c("Xena", "Piccolo","Recount2", 
               "MSKCC","MSKCC Batch")
...
gtex_data = rbind(
  cbind( Value= gtex_mat[,"corr_XENA_Toil"], Group= rep("Xena", nrow(gtex_mat))) ,
...
group_levels2=c("Xena", "Recount2","MSKCC", "MSKCC Batch")
...
gtex_cor=ggplot(gtex_data, aes(x = Group, y = Value, fill=Group)) +
  ...
  guides(fill=FALSE) +
  annotate("text", label = length(which(gtex_mat[,"corr_MSKCC_BATCH"] < 0.8))  ,
...
prot_data = rbind(
  ...
  cbind( Value= mat[,"corr_protein_Xena_Toil"], Group= rep("Xena", nrow(mat)))   )

group_levels3=c("GDC", "Xena", "Piccolo", "Recount2","MSKCC", "MSKCC Batch")
...
dodge <- position_dodge(width = 0.4)
prot1 =   ggplot(beeswarm, aes(x, y, colour = DataSource)) +
...
nat3= list(tcga_cor, gtex_cor, prot1)
